...
function setupSplitDropdown(toggleId, dropdownId, onSelect) {
  ...
  toggle.addEventListener("click", (e) => {
    e.stopPropagation();
    // Close any other open dropdown or popover
...
window.addEventListener("resize", () => {
  ...
  resizeTimer = setTimeout(() => {
    ...
    computeGridSize();
    if (state.gridRows !== oldRows || state.gridCols !== oldCols) {
      buildGrid();
      // Re-render existing beads that are still in bounds
      for (const [key, color] of state.beads) {
        const [r, c] = key.split(",").map(Number);
        if (inBounds(r, c)) {
          const cell = getCell(r, c);
          if (cell) renderBead(cell, color, state.beadShapes.get(key));
        }
      }
      redrawAll();
    } else {
